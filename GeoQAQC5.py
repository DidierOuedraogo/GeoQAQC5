...
# Onglets
tabs = st.tabs(["Type de Contrôle", "Importation des Données", "Mappage des Colonnes", "Analyse", "Export"])
    ...
    with control_subtabs[1]:
        ...
        if control_type == "Standards CRM":
            ...
            if st.button("Utiliser ces données d'exemple", key="use_crm_example"):
                ...
                st.session_state.reference_value = 1.25
                st.session_state.reference_stddev = 0.05
                st.session_state.tolerance_percent = 10.0
                st.session_state.active_tab = 2  # Aller directement au mappage des colonnes
    ...
    if import_method == "Téléchargement de fichier":
        ...
        if control_type == "Standards CRM":
            ...
            if st.button("Utiliser ces données d'exemple", key="use_crm_example_import"):
                ...
                if 'reference_value' not in st.session_state:
                    st.session_state.reference_value = 1.25
                if 'reference_stddev' not in st.session_state:
                    st.session_state.reference_stddev = 0.05
                if 'tolerance_percent' not in st.session_state:
                    st.session_state.tolerance_percent = 10.0
            ...
            if st.button("Générer la Carte de Contrôle"):
                ...
                if analysis_data.empty:
                    ...
                    if lower_limit is not None and upper_limit is not None:
                        ...
                        # Bouton pour naviguer vers l'onglet d'exportation
                        if st.button("Continuer vers l'exportation", key="continue_to_export"):
...
# Dans le cinquième onglet - Export
with tabs[4]:
    st.header("Exportation des Résultats")
    
    if st.session_state.current_fig is None:
        st.warning("Aucun résultat à exporter. Veuillez d'abord générer une analyse dans l'onglet 'Analyse'.")
    else:
